...
class TopicUnreadViewTest(TestCase):
    ...
    def test_topic_unread_list(self):
        ...
        utils.login(self)
        response = self.client.get(reverse('forum:topic:unread:index'))
        self.assertEqual(list(response.context['page']), [self.topic2, self.topic])

        # fake next page
        response = self.client.get(reverse('forum:topic:unread:index') + "?topic_id=" + str(self.topic2.pk))
    ...
    def test_topic_unread_list_invalid_topic_id(self):
        ...
        response = self.client.get(reverse('forum:topic:unread:index') + "?topic_id=" + str(last_pk + 1))
    ...
    def test_topic_unread_list_empty_page(self):
        ...
        """
        utils.login(self)
        response = self.client.get(reverse('forum:topic:unread:index') + "?topic_id=" + str(self.topic.pk))
        self.assertEqual(response.status_code, 404)
