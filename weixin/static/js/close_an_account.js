...
function action_create_ticket(openid, total_money, url){
	...
	if(ticket_value){
		if(auth_code){
			if(flag){
				var person_wallet, action, ticket_value;
				var xmlhttp;
				person_wallet = document.getElementById("person_wallet").value;
				auth_code = document.getElementById("auth_code").value;
				action = 'ajax_create_ticket';
				var data = '{"openid":"OPENID", "action":"ACTION", "person_wallet":"PERSON_WALLET", "total_money":"TOTAL_MONEY","ticket_value":"TICKET_VALUE","auth_code":"AUTH_CODE"}';
				data = data.replace(/OPENID/, openid).replace(/ACTION/, action).replace(/PERSON_WALLET/, person_wallet).replace(/TOTAL_MONEY/,total_money).replace(/TICKET_VALUE/,ticket_value).replace(/AUTH_CODE/, auth_code);
