...
$.unfunkyUI = function(opts) {
			remove(window.top, opts); 
...
function install(el, opts) {
    ...
    var z = opts.baseZ;
		...
		topWin.$.DialogData.cw =window.screen.width;
;//窗体高度

		topWin.$.DialogData.ch =window.screen.height;//窗体宽度//必须考虑文本框处于页面边缘处，控件显示不全的问题
		
        topWin.$.DialogData.sw = 0;//横向滚动条位置
		topWin.$.DialogData.sh = 0;//纵向滚动条位置//考虑滚动的情况
    ...
    if(!css.height){
		...
		if(css.top==0){
			...
			css.top = (topWin.$.DialogData.ch - css.height - 130) / 2 + topWin.$.DialogData.sh - 8;//有8像素的透明背景
    ...
    var arr = [];
		...
		if(opts.showDialog){
			...
			arr.push("           	<input id='_ButtonCancel_"+opts.id+"' style='cursor:pointer; border:1px solid #d7d7d7;' type='button' onclick=\"$.unfunkyUI('"+opts+"');\" value='关闭'>");
			arr.push("            </div></td></tr>");
			arr.push("      </table></td>");
	   ...
	   //判断是否是IE6
		if(ie6){	
		lyr2 = full ? $('<div class="funkyUI blockOverlay" style="z-index:'+ z++ +';cursor:none;border:none;margin:0;padding:0;width:100%;height:100%;top:0;left:0"><iframe src="" style="position:absolute; visibility:inherit; top:0px; left:0px; width:100%; height:100%; z-index:-1;"></iframe></div>', topWin.document.body):$('<div class="funkyUI blockOverlay" style="z-index:'+ z++ +';cursor:none;border:none;margin:0;padding:0;width:100%;height:100%;top:0;left:0"><iframe src="" style="position:absolute; visibility:inherit; top:0px; left:0px; width:100%; height:100%; z-index:-1;"></iframe></div>');
	
		}else{
		lyr2 = full ? $('<div class="funkyUI blockOverlay" style="z-index:'+ z++ +';cursor:none;border:none;margin:0;padding:0;width:100%;height:100%;top:0;left:0"></div>', topWin.document.body):$('<div class="funkyUI blockOverlay" style="z-index:'+ z++ +';cursor:none;border:none;margin:0;padding:0;width:100%;height:100%;top:0;left:0"></div>');
