    ...
    !function (a) {
        ...
        d = function () {
            ...
            return c.prototype.uid = function () {
                ...
                var a, b;
                if (a = this.$inputor.caret({iframe: this.app.iframe}).caret("offset", this.pos - 1))return "true" === this.$inputor.attr("contentEditable") && (a = this.cur_rect || (this.cur_rect = a) || a), b = this.app.document.selection ? 0 : 2, {
                    left: a.left,
                    top: a.top,
            ...
            return b.prototype.init = function () {
                var a;
                return a = this.context.get_opt("alias") || this.context.at.charCodeAt(0), this.$el.attr({id: "at-view-" + a})
                ...
                for (this.$el.find("ul").empty(), d = this.$el.find("ul"), g = this.context.get_opt("tpl"), h = 0, i = b.length; i > h; h++)e = b[h], e = a.extend({}, e, {"atwho-at": this.context.at}), f = this.context.callbacks("tpl_eval").call(this.context, g, e), c = a(this.context.callbacks("highlighter").call(this.context, f, this.context.query.text)), c.data("item-data", e), d.append(c);
            ...
            before_save: function (b) {
                ...
                for (f = [], d = 0, e = b.length; e > d; d++)c = b[d], f.push(a.isPlainObject(c) ? c : {name: c});
                return f
                ...
                return g
            }, remote_filter: null, sorter: function (a, b, c) {
                var d, e, f, g;
                if (!a)return b;
...
!function () {
    ...
    a.bullet = /(?:[*+-]|\d+\.)/, a.item = /^( *)(bull) [^\n]*(?:\n(?!\1bull )[^\n]*)*/, a.item = h(a.item, "gm")(/bull/g, a.bullet)(), a.list = h(a.list)(/bull/g, a.bullet)("hr", /\n+(?=(?: *[-*_]){3,} *(?:\n+|$))/)(), a._tag = "(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:/|@)\\b", a.html = h(a.html)("comment", /<!--[\s\S]*?-->/)("closed", /<(tag)[\s\S]+?<\/\1>/)("closing", /<tag(?:"[^"]*"|'[^']*'|[^'">])*?>/)(/tag/g, a._tag)(), a.paragraph = h(a.paragraph)("hr", a.hr)("heading", a.heading)("lheading", a.lheading)("blockquote", a.blockquote)("tag", "<" + a._tag)("def", a.def)(), a.normal = j({}, a), a.gfm = j({}, a.normal, {
        ...
        for (var d, e, f, g, h, i, j, k, l, b = b.replace(/^ +$/gm, ""); b;)if ((f = this.rules.newline.exec(b)) && (b = b.substring(f[0].length), f[0].length > 1 && this.tokens.push({type: "space"})), f = this.rules.code.exec(b))b = b.substring(f[0].length), f = f[0].replace(/^ {4}/gm, ""), this.tokens.push({
            type: "code",
            ...
            depth: "=" === f[2] ? 1 : 2,
            text: f[1]
        }); else if (f = this.rules.hr.exec(b))b = b.substring(f[0].length), this.tokens.push({type: "hr"}); else if (f = this.rules.blockquote.exec(b))b = b.substring(f[0].length), this.tokens.push({type: "blockquote_start"}), f = f[0].replace(/^ *> ?/gm, ""), this.token(f, c), this.tokens.push({type: "blockquote_end"}); else if (f = this.rules.list.exec(b)) {
            for (b = b.substring(f[0].length), g = f[2], this.tokens.push({
                type: "list_start",
                ordered: g.length > 1
            }), f = f[0].match(this.rules.item), d = !1, l = f.length, k = 0; l > k; k++)i = f[k], j = i.length, i = i.replace(/^ *([*+-]|\d+\.) +/, ""), ~i.indexOf("\n ") && (j -= i.length, i = this.options.pedantic ? i.replace(/^ {1,4}/gm, "") : i.replace(new RegExp("^ {1," + j + "}", "gm"), "")), this.options.smartLists && k !== l - 1 && (h = a.bullet.exec(f[k + 1])[0], g === h || g.length > 1 && h.length > 1 || (b = f.slice(k + 1).join("\n") + b, k = l - 1)), e = d || /\n\n(?!\s*$)/.test(i), k !== l - 1 && (d = "\n" === i.charAt(i.length - 1), e || (e = d)), this.tokens.push({type: e ? "loose_item_start" : "list_item_start"}), this.token(i, !1), this.tokens.push({type: "list_item_end"});
    ...
    c._inside = /(?:\[[^\]]*\]|[^\[\]]|\](?=[^\[]*\]))*/, c._href = /\s*<?([\s\S]*?)>?(?:\s+['"]([\s\S]*?)['"])?\s*/, c.link = h(c.link)("inside", c._inside)("href", c._href)(), c.reflink = h(c.reflink)("inside", c._inside)(), c.normal = j({}, c), c.pedantic = j({}, c.normal, {
        ...
        switch (this.token.type) {
            ...
            case"list_item_start":
                for (var b = ""; "list_item_end" !== this.next().type;)b += "text" === this.token.type ? this.parseText() : this.tok();
...
(function () {
    ...
    is_end_of_tag = function (char) {
        ...
        for (best_match_in_before = start_in_before, best_match_in_after = start_in_after, best_match_length = 0, match_length_at = {}, index_in_before = i = ref = start_in_before, ref1 = end_in_before; ref1 >= ref ? ref1 > i : i > ref1; index_in_before = ref1 >= ref ? ++i : --i) {
            for (new_match_length_at = {}, looking_for = before_tokens[index_in_before], locations_in_after = index_of_before_locations_in_after_tokens[looking_for], j = 0, len = locations_in_after.length; len > j; j++)if (index_in_after = locations_in_after[j], !(start_in_after > index_in_after)) {
        ...
        for (index = {}, ref = p.find_these, i = 0, len = ref.length; len > i; i++)for (token = ref[i], index[token] = [], idx = p.in_these.indexOf(token); -1 !== idx;)index[token].push(idx), idx = p.in_these.indexOf(token, idx + 1);
        return index
        ...
        for (post_processed = [], last_op = {action: "none"}, is_single_whitespace = function (op) {
...
(function () {
    ...
    $ = jQuery, EditorImageUpload = function () {
        ...
        return EditorImageUpload.prototype.defaults = {
            ...
            //var placeholder;
            //return placeholder = $.format("![" + this.options.placeholderText + "]()", {image_name: file.name}), this.el.val(this.el.val() + placeholder), placeholder
            ...
            var $el = $.format('<li class="item"><img src="{src}" cmid="{cmid}" style="width:150px;height:150px;float:left;"></li>',{
                src:data.url,
                cmid:data.cmid
              });
              $("#imageslistpanel").prepend($el);
             var imagelist=[];
              $.each($("#imageslistpanel li"),function(idx,val){
                imagelist.push($(val).find("img").attr("cmid"));
              });
              this.el.closest(".js-reply").find("input[name=imagelist]").val(imagelist.join(','));
...
(function () {
    ...
    $ = jQuery, Mark = function () {
        ...
        return Bookmark.prototype.defaults = {
            ...
            return this.el.waypoint(this.onWaypoint, {offset: "100%"})
...
(function () {
    ...
    $ = jQuery, $.extend({
        emoji_list: function () {
            return [{'name':'微笑','name_raw':'5c/huanglianwx_thumb.gif'},
{'name':'嘻嘻','name_raw':'0b/tootha_thumb.gif'},
{'name':'哈哈','name_raw':'6a/laugh.gif'},
{'name':'可爱','name_raw':'14/tza_thumb.gif'},
{'name':'可怜','name_raw':'af/kl_thumb.gif'},
{'name':'挖鼻','name_raw':'0b/wabi_thumb.gif'},
{'name':'吃惊','name_raw':'f4/cj_thumb.gif'},
{'name':'害羞','name_raw':'6e/shamea_thumb.gif'},
{'name':'挤眼','name_raw':'c3/zy_thumb.gif'},
{'name':'闭嘴','name_raw':'29/bz_thumb.gif'},
{'name':'鄙视','name_raw':'71/bs2_thumb.gif'},
{'name':'爱你','name_raw':'6d/lovea_thumb.gif'},
{'name':'泪','name_raw':'9d/sada_thumb.gif'},
{'name':'偷笑','name_raw':'19/heia_thumb.gif'},
{'name':'亲亲','name_raw':'8f/qq_thumb.gif'},
{'name':'生病','name_raw':'b6/sb_thumb.gif'},
{'name':'太开心','name_raw':'58/mb_thumb.gif'},
{'name':'白眼','name_raw':'d9/landeln_thumb.gif'},
{'name':'右哼哼','name_raw':'98/yhh_thumb.gif'},
{'name':'左哼哼','name_raw':'6d/zhh_thumb.gif'},
{'name':'嘘','name_raw':'a6/x_thumb.gif'},
{'name':'衰','name_raw':'af/cry.gif'},
{'name':'委屈','name_raw':'73/wq_thumb.gif'},
{'name':'吐','name_raw':'9e/t_thumb.gif'},
{'name':'哈欠','name_raw':'cc/haqianv2_thumb.gif'},
{'name':'抱抱','name_raw':'27/bba_thumb.gif'},
{'name':'怒','name_raw':'7c/angrya_thumb.gif'},
{'name':'疑问','name_raw':'5c/yw_thumb.gif'},
{'name':'馋嘴','name_raw':'a5/cza_thumb.gif'},
{'name':'拜拜','name_raw':'70/88_thumb.gif'},
{'name':'思考','name_raw':'e9/sk_thumb.gif'},
{'name':'汗','name_raw':'24/sweata_thumb.gif'},
{'name':'困','name_raw':'40/kunv2_thumb.gif'},
{'name':'睡','name_raw':'96/huangliansj_thumb.gif'},
{'name':'钱','name_raw':'90/money_thumb.gif'},
{'name':'失望','name_raw':'0c/sw_thumb.gif'},
{'name':'酷','name_raw':'40/cool_thumb.gif'},
{'name':'色','name_raw':'20/huanglianse_thumb.gif'},
{'name':'哼','name_raw':'49/hatea_thumb.gif'},
{'name':'鼓掌','name_raw':'36/gza_thumb.gif'},
{'name':'晕','name_raw':'d9/dizzya_thumb.gif'},
{'name':'悲伤','name_raw':'1a/bs_thumb.gif'},
{'name':'抓狂','name_raw':'62/crazya_thumb.gif'},
{'name':'黑线','name_raw':'91/h_thumb.gif'},
{'name':'阴险','name_raw':'6d/yx_thumb.gif'},
{'name':'怒骂','name_raw':'60/numav2_thumb.gif'},
{'name':'互粉','name_raw':'89/hufen_thumb.gif'},
{'name':'心','name_raw':'40/hearta_thumb.gif'},
{'name':'伤心','name_raw':'ea/unheart.gif'},
{'name':'猪头','name_raw':'58/pig.gif'},
{'name':'熊猫','name_raw':'6e/panda_thumb.gif'},
{'name':'兔子','name_raw':'81/rabbit_thumb.gif'},
{'name':'ok','name_raw':'d6/ok_thumb.gif'},
{'name':'耶','name_raw':'d9/ye_thumb.gif'},
{'name':'good','name_raw':'d8/good_thumb.gif'},
{'name':'NO','name_raw':'ae/buyao_org.gif'},
{'name':'赞','name_raw':'d0/z2_thumb.gif'},
{'name':'来','name_raw':'40/come_thumb.gif'},
{'name':'弱','name_raw':'d8/sad_thumb.gif'},
{'name':'草泥马','name_raw':'7a/shenshou_thumb.gif'},
{'name':'神马','name_raw':'60/horse2_thumb.gif'},
{'name':'囧','name_raw':'15/j_thumb.gif'},
{'name':'浮云','name_raw':'bc/fuyun_thumb.gif'},
{'name':'给力','name_raw':'1e/geiliv2_thumb.gif'},
{'name':'围观','name_raw':'f2/wg_thumb.gif'},
{'name':'威武','name_raw':'70/vw_thumb.gif'},
{'name':'奥特曼','name_raw':'bc/otm_thumb.gif'},
{'name':'礼物','name_raw':'c4/liwu_thumb.gif'},
{'name':'钟','name_raw':'d3/clock_thumb.gif'},
{'name':'话筒','name_raw':'9f/huatongv2_thumb.gif'},
{'name':'蜡烛','name_raw':'d9/lazhuv2_thumb.gif'},
{'name':'蛋糕','name_raw':'3a/cakev2_thumb.gif'}]
        }
    })
}).call(this);
(function () {
    ...
    $ = jQuery, Like = function () {
        ...
        return Like.prototype.defaults = {
            ...
            var post;
            this.isSending || (this.isSending = !0, post = $.post(this.el.attr("href"), {csrfmiddlewaretoken: this.options.csrfToken}), post.done(function (_this) {
                return function (data) {
            ...
            return this.el.attr("href", data.url_delete), count = this.el.data("count"), count += 1, this.el.data("count", count), removeLikeText = $.format(this.options.removeLikeText, {count: count}), this.el.text(removeLikeText)
        }, Like.prototype.removeLike = function (data) {
            var count, likeText;
            return this.el.attr("href", data.url_create), count = this.el.data("count"), count -= 1, this.el.data("count", count), likeText = $.format(this.options.likeText, {count: count}), this.el.text(likeText)
...
(function () {
    ...
    $ = jQuery, Postify = function () {
        ...
        return Postify.prototype.defaults = {csrfToken: "csrf_token"}, Postify.prototype.setUp = function () {
