...
class UserViewTest(TestCase):
    ...
    def test_login_email(self):
        ...
        """
        # get
        response = self.client.get(reverse('user:auth:login'))
        self.assertEqual(response.status_code, 200)

        # post
        form_data = {'username': self.user.email, 'password': "bar"}
        response = self.client.post(reverse('user:auth:login'),
                                    form_data)
    ...
    def test_login_redirect(self):
        ...
        utils.login(self)
        response = self.client.get(reverse('user:auth:login'))
        expected_url = self.user.jf.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)
        # next
        response = self.client.get(reverse('user:auth:login') + '?next=/fakepath/')
    ...
    def test_register(self):
        ...
        """
        # get
        response = self.client.get(reverse('user:auth:register'))
        self.assertEqual(response.status_code, 200)

        # post
        form_data = {'username': 'uniquefoo', 'email': 'some@some.com',
                     'email2': 'some@some.com', 'password': 'pass'}
        response = self.client.post(reverse('user:auth:register'),
                                    form_data)
        expected_url = reverse('user:auth:login')
        self.assertRedirects(response, expected_url, status_code=302)

        # redirect logged in user
        utils.login(self)
        response = self.client.get(reverse('user:auth:register'))
        self.assertRedirects(response, reverse('user:update'), status_code=302)
    ...
    def test_register_email_sent(self):
        ...
        form_data = {'username': 'uniquefoo', 'email': 'some@some.com',
                     'email2': 'some@some.com', 'password': 'pass'}
        response = self.client.post(reverse('user:auth:register'), form_data)
    ...
    def test_register_next_logged_in(self):
        ...
        # redirect logged in user
        utils.login(self)
        response = self.client.get(reverse('user:auth:register') + "?next=/fakepath/")
        self.assertRedirects(response, '/fakepath/', status_code=302, target_status_code=404)

    @override_settings(ROOT_URLCONF='forum.user.auth.tests.urls')
    ...
    def test_login_rate_limit(self):
        ...
        for attempt in range(5):
            ...
            self.assertTemplateUsed(response, 'forum/user/auth/login.html')

        url = reverse('user:auth:login') + "?next=/path/"
        response = self.client.post(url, form_data)
    ...
    def test_custom_reset_password(self):
        ...
        for attempt in range(5):
            response = self.client.post(reverse('user:auth:password-reset'), form_data)
            expected_url = reverse("user:auth:password-reset-done")
            self.assertRedirects(response, expected_url, status_code=302)

        response = self.client.post(reverse('user:auth:password-reset'), form_data)
        expected_url = reverse("user:auth:password-reset")
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_admin_login(self):
        ...
        expected_url = reverse("user:auth:login") + "?next=" + reverse('admin:login')
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_registration_activation(self):
        ...
        response = self.client.get(
            reverse(
                'user:auth:registration-activation',
    ...
    def test_registration_activation_invalid(self):
        ...
        Consumer.objects.filter(user__pk=self.user.pk).update(is_verified=True)
        response = self.client.get(
            reverse(
                'user:auth:registration-activation',
                kwargs={'pk': self.user.pk, 'token': token}
    ...
    def test_resend_activation_email(self):
        ...
        form_data = {'email': user.email,
                     'password': "foo"}
        response = self.client.post(reverse('user:auth:resend-activation'),
                                    form_data)
        expected_url = reverse("user:auth:login")
        self.assertRedirects(response, expected_url, status_code=302)
        self.assertEquals(len(mail.outbox), 1)
        ...
        # get
        response = self.client.get(reverse('user:auth:resend-activation'))
    ...
    def test_logout(self):
        ...
        response = self.client.get(reverse('user:auth:logout'))
        self.assertEqual(response.status_code, 200)
        ...
        utils.login(self)
        self.assertTrue(self.client.session.items())
        response = self.client.post(reverse('user:auth:logout') + '?next=/fakepath/')
        self.assertRedirects(response, '/fakepath/', status_code=302, target_status_code=404)
        self.assertFalse(self.client.session.items())
    ...
    def test_logout_anonymous_redirect(self):
        ...
        response = self.client.get(reverse('user:auth:logout') + '?next=/fakepath/')
        self.assertRedirects(response, '/fakepath/', status_code=302, target_status_code=404)
