...
def custom_login(request, **kwargs):
    ...
    if request.user.is_authenticated():
        return redirect(request.GET.get('next', request.user.jf.get_absolute_url()))
...
def custom_logout(request, **kwargs):
    ...
    if not request.user.is_authenticated():
        return redirect(request.GET.get('next', reverse('user:auth:login')))

    if request.method == 'POST':
        return logout(request, **kwargs)

    return render(request, 'forum/user/auth/logout.html')


@ratelimit(field='email', rate='5/5m')
def custom_password_reset(request, **kwargs):
    if request.method == "POST" and request.is_limited:
        return redirect(reverse("user:auth:password-reset"))
...
def register(request, registration_form=RegistrationForm):
    if request.user.is_authenticated():
        return redirect(request.GET.get('next', reverse('user:update')))
    ...
    if request.method == 'POST':
        ...
        if not request.is_limited and form.is_valid():
            ...
            return redirect(reverse('user:auth:login'))
    else:
        form = registration_form()

    context = {'form': form, }

    return render(request, 'forum/user/auth/register.html', context)
...
def resend_activation_email(request):
    ...
    return render(request, 'forum/user/auth/activation_resend.html', context)
