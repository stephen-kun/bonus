...
def update(request):
    if request.method == 'POST':
        ...
        form = UserProfileForm(data=request.POST, instance=request.user.jf)
        ...
        form = UserProfileForm(instance=request.user.jf)
    ...
    return render(request, 'forum/user/profile_update.html', context)
...
def password_change(request):
    ...
    return render(request, 'forum/user/profile_password_change.html', context)
...
def email_change(request):
    ...
    return render(request, 'forum/user/profile_email_change.html', context)
...
def _activity(request, pk, slug, queryset, template, reverse_to, context_name, per_page):
    ...
    if p_user.jf.slug != slug:
        url = reverse(reverse_to, kwargs={'pk': p_user.pk, 'slug': p_user.jf.slug})
...
def topics(request, pk, slug):
    ...
    return _activity(
        ...
        template='forum/user/profile_topics.html',
...
def comments(request, pk, slug):
    ...
    return _activity(
        ...
        template='forum/user/profile_comments.html',
...
def likes(request, pk, slug):
    ...
    return _activity(
        request, pk, slug,
        queryset=user_comments,
        template='forum/user/profile_likes.html',
...
@login_required
def menu(request):
    return render(request, 'forum/user/menu.html')
