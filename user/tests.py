...
class UserViewTest(TestCase):
    ...
    def test_user_views_denied_to_non_logged_users(self):
        ...
        """
        pk = self.user.pk
        slug = self.user.jf.slug

        response = self.client.get(reverse('user:topics', kwargs={'pk': pk, 'slug': slug}))
        self.assertEqual(response.status_code, 302)
        response = self.client.get(reverse('user:detail', kwargs={'pk': pk, 'slug': slug}))
        self.assertEqual(response.status_code, 302)
        response = self.client.get(reverse('user:likes', kwargs={'pk': pk, 'slug': slug}))
        self.assertEqual(response.status_code, 302)
        ...
        self.assertEqual(response.status_code, 302)
        response = self.client.get(reverse('user:email-change'))
        self.assertEqual(response.status_code, 302)
        response = self.client.get(reverse('user:email-change-confirm', kwargs={'token': "foo"}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_profile_topics(self):
        ...
        """
        utils.login(self)
        response = self.client.get(reverse("user:topics", kwargs={'pk': self.user2.pk,
                                                                            'slug': self.user2.jf.slug}))
    ...
    def test_profile_topics_invalid_slug(self):
        ...
        response = self.client.get(reverse("user:topics", kwargs={'pk': self.user2.pk,
                                                                            'slug': "invalid"}))
        expected_url = reverse("user:topics", kwargs={'pk': self.user2.pk,
                                                                'slug': self.user2.jf.slug})
    ...
    def test_profile_comments_invalid_slug(self):
        ...
        response = self.client.get(reverse("user:detail", kwargs={'pk': self.user2.pk,
                                                                            'slug': "invalid"}))
        expected_url = reverse("user:detail", kwargs={'pk': self.user2.pk,
                                                                'slug': self.user2.jf.slug})
        self.assertRedirects(response, expected_url, status_code=301)
    ...
    def test_profile_likes(self):
        ...
        Like.objects.create(user=self.user, comment=comment2)
        response = self.client.get(reverse("user:likes", kwargs={'pk': self.user2.pk,
                                                                           'slug': self.user2.jf.slug}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_profile_likes_order(self):
        ...
        like_a = Like.objects.create(user=self.user2, comment=comment_a)
        Like.objects.create(user=self.user2, comment=comment_b)
        like_c = Like.objects.create(user=self.user2, comment=comment_c)

        Like.objects.filter(pk=like_a.pk).update(date=timezone.now() - datetime.timedelta(days=10))
        Like.objects.filter(pk=like_c.pk).update(date=timezone.now() - datetime.timedelta(days=5))
    ...
    def test_profile_likes_dont_show_removed_or_private(self):
        ...
        Like.objects.create(user=self.user2, comment=comment_a)
        Like.objects.create(user=self.user2, comment=comment_b)
        Like.objects.create(user=self.user2, comment=comment_c)
        Like.objects.create(user=self.user2, comment=comment_d)
        Like.objects.create(user=self.user2, comment=comment_e)
    ...
    def test_profile_likes_invalid_slug(self):
        ...
        response = self.client.get(reverse("user:likes", kwargs={'pk': self.user2.pk,
                                                                           'slug': "invalid"}))
        expected_url = reverse("user:likes", kwargs={'pk': self.user2.pk,
                                                               'slug': self.user2.jf.slug})
        self.assertRedirects(response, expected_url, status_code=301)
    ...
    def test_profile_likes_paginate(self):
        ...
        comment = utils.create_comment(user=self.user2, topic=self.topic)
        comment2 = utils.create_comment(user=self.user2, topic=self.topic)
        Like.objects.create(user=self.user2, comment=comment)
        like = Like.objects.create(user=self.user2, comment=comment2)
    ...
    def test_profile_update(self):
        ...
        form_data = {'first_name': 'foo', 'last_name': 'bar',
                     'location': 'forum', 'timezone': self.user.jf.timezone}
        response = self.client.post(reverse('user:update'),
                                    form_data)
        expected_url = reverse('user:update')
    ...
    def test_profile_password_change_re_login(self):
        ...
        form_data = {'old_password': 'foo',
                     'new_password1': 'bar',
                     'new_password2': 'bar'}
        response = self.client.post(reverse('user:password-change'), form_data)
        expected_url = reverse("user:update")
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_email_change_confirm(self):
        ...
        response = self.client.get(reverse('user:email-change-confirm', kwargs={'token': token}))
        expected_url = reverse("user:update")
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_email_change_duplicated(self):
        ...
        token = UserEmailChangeTokenGenerator().generate(self.user, new_email)
        self.client.get(reverse('user:email-change-confirm', kwargs={'token': token}))
        self.assertEqual(User.objects.get(pk=self.user.pk).email, old_email)

    @override_settings(UNIQUE_EMAILS=False)
    ...
    def test_profile_email_change(self):
        ...
        response = self.client.post(reverse('user:email-change'),
                                    form_data)
        expected_url = reverse("user:update")
        self.assertRedirects(response, expected_url, status_code=302)
...
class UserFormTest(TestCase):
    ...
    def test_profile(self):
        ...
        form_data = {'first_name': 'foo', 'last_name': 'bar',
                     'location': 'forum', 'timezone': self.user.jf.timezone}
        form = UserProfileForm(data=form_data, instance=self.user.jf)
    ...
    @override_settings(CASE_INSENSITIVE_EMAILS=True)
    ...
    @override_settings(CASE_INSENSITIVE_EMAILS=False)
...
class UserModelTest(TestCase):
    ...
    def test_user_superuser(self):
        ...
        user.save()
        self.assertTrue(user.jf.is_administrator)
        self.assertTrue(user.jf.is_moderator)
...
class UtilsUserTests(TestCase):
    ...
    def test_user_activation_token_generator(self):
        ...
        """
        self.user.jf.is_verified = False
    ...
    def test_user_activation_email(self):
        ...
        def monkey_sender(request, subject, template_name, context, email):
            ...
            self.assertEqual(template_name, 'forum/user/activation_email.html')
    ...
    def test_email_change_email(self):
        ...
        def monkey_sender(request, subject, template_name, context, email):
            ...
            self.assertEqual(template_name, 'forum/user/email_change_email.html')
