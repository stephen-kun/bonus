...
class CommentForm(forms.ModelForm):
    ...
    def _get_comment_html(self):
        ...
        markdown = Markdown(no_follow=not user.jf.is_moderator)
...
class WXCommentForm(forms.ModelForm):
    ...
    def __init__(self, user=None, topic=None, *args, **kwargs):
        super(WXCommentForm, self).__init__(*args, **kwargs)
        self.user = user
        self.topic = topic
        self.mentions = None  # {username: User, }
        # self.polls = None  # {polls: [], choices: []}
    ...
    def save(self, commit=True):
        ...
        self.instance.comment_html = self._get_comment_html()
        comment = super(WXCommentForm, self).save(commit)
        return comment
...
class CommentImageForm(forms.Form):
    ...
    def save(self):
        ...
        upload_to = os.path.join('forum', 'images', str(self.user.pk))
