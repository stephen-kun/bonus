...
def update(request, pk):
    ...
    return render(request, 'forum/comment/update.html', context)
...
def delete(request, pk, remove=True):
    ...
    return render(request, 'forum/comment/moderate.html', context)
...
def image_upload_ajax(request):
    ...
    if form.is_valid():
        image = form.save()
        cimage = CommentImages.objects.create(imageurl = image.url)
        return json_response({'url': image.url, 'cmid':cimage.id})
